...
- **Indexing API scope limitation (discovered 2026-04-19 during R19):** `submit-url-for-indexing` calls Google's Indexing API, which is officially restricted to `JobPosting` and `BroadcastEvent` schema types. Calls for any other URL (band pages, articles, landing pages) return 200 with a silent no-op — the Pipedream connector surfaces this as a `null` response. For re-indexing band URLs after content edits, fall back to either (a) GSC web UI's URL Inspection tool "Request indexing" button (works for arbitrary URLs, separate mechanism from the Indexing API) or (b) waiting for Google's normal re-crawl cadence (typically 1-3 weeks for a site with this authority level). Do not assume `submit-url-for-indexing` did anything useful for a band URL.
