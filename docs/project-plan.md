...
_Last updated: 2026-04-19 · 8:20 AM Central_
...
**Heads-up for the NEXT thread specifically:** top-of-funnel foundation work (B7 Part 2, B1, R11, B6) is still untouched from prior sessions — this session was cleanup only, not foundation. R4 remains on ice until 2026-05-03 (waiting for Google to re-crawl after this session's /home 301). Open question from end of session: Ray floated connecting the `google_search_console__pipedream` connector so Jarvis can do R3/R4 verification work directly instead of Ray playing eyes-and-fingers in the GSC UI. Connector exists, is DISCONNECTED, supports sitemap management + URL Inspection (2000/day, 600/min) + Search Analytics. Recommended to connect at the start of next session before R4 reassessment on 5/3.
...
5. **R4 - Wildcard 301s.** See [roadmap.md#r4](roadmap.md). Reassess 2026-05-03 after Google re-crawls following this session's `/home` 301. Connecting GSC connector (see Pick-up block) would let Jarvis do this sweep directly.
