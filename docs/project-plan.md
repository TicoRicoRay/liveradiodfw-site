...
_Last updated: 2026-04-21 · 12:44 PM Central_
...
**Context (2026-04-21 session, mid-flight):** Working B7 Part 2 — library/runner split to get `sync_calendar.py` off `gh-pages`. Drafted in workspace: `sync_lib.py` (479 lines, pure functions, no secrets, destined for `gh-pages`), `sync_runner.py` (401 lines, orchestration + secrets + git push, destined for Ray's Windows box, NOT the repo), and `.env.example` (template). Still open: update 6 caller imports (`test_is_private_event.py`, `test_description_handling.py`, `test_cancellation_reschedule.py`, `import_historic.py`, `import_bandzoogle.py`, `fetch_historic.py` — change `from sync_calendar import ...` to `from sync_lib import ...`); write `runbooks/windows-sync-task.md`; package off-repo files for Ray to install; commit `sync_lib.py` to `gh-pages` and delete `sync_calendar.py`; verify `https://www.liveradiodfw.com/sync_calendar.py` 404s and GitHub raw no longer exposes the passphrase. Two open decisions deferred: (a) delete `sync_calendar.py` outright vs. leave a deprecated stub that points to the new runbook; (b) runbook location — `docs/runbooks/windows-sync-task.md` only, or also paste a copy in the owning Perplexity thread so it survives thread drift.
...
0. **B7 Part 2 is mid-flight — resume from workspace drafts.** `sync_lib.py`, `sync_runner.py`, `.env.example` are already drafted in the last session's workspace but NOT committed. If that workspace is gone by the time this is read, the drafts will need to be re-generated from `gh-pages/sync_calendar.py` using the library/runner split pattern Ray approved 2026-04-21 (decisions 1a/2a/3a: Windows Task Scheduler host, library/runner split, `.env` file for secrets). The 9-step todo list and two unanswered decisions (old file disposition, runbook location) are captured in the mid-flight context block above.

1. **R23 - Preserve the Monthly Profile Audit venue-discovery cron.** See [roadmap.md#r23](roadmap.md). Filed 2026-04-21. Next fires ~2026-05-01; that is the last easy checkpoint before another month of drift. Forensics the "More Band Marketing" Perplexity thread for the task prompt BEFORE the thread is lost. First band-marketing project Ray built with Jarvis — do not lose.

2. **B7 - Part 2: move `sync_calendar.py` off `gh-pages`.** See [bugs.md#b7](bugs.md). Passphrase was rotated 2026-04-17 PM (Part 1), but the new value is still hard-coded in `gh-pages/sync_calendar.py` and therefore still fetchable at `https://www.liveradiodfw.com/sync_calendar.py` and GitHub raw. Relocate the sync script to a non-public host (likely Ray's Windows box; see B1) and read the passphrase from an env var or `.gitignore`d config file.
