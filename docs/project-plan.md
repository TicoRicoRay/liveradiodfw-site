...
## READ FIRST NEXT SESSION - Ray's directive (2026-04-24 end of session)

> **Critical platform context:** Perplexity Computer silently drops on submit in every new thread as of ~2026-04-17. Ray cannot start a new session. Support has been unresponsive for 3+ weeks. If you are reading this from a NEW thread that is actually working, threading has been restored - celebrate briefly, then proceed.
>
> **If threading is still broken:** everything in this file and the repos is self-contained. Windows-box cron continues to run daily 8:00 AM Central without agent intervention. Site keeps serving. No action required from the agent side.
>
> **Ray's standing directive from 2026-04-23 still applies:** "Assume this buggy mousetrap we've built is NOT working. Test before proceeding."

## Bug tracking drift - action item for next working session

**2026-04-24 session revealed that `bugs.md` had accumulated six drift instances** where a bug's `**Status:**` line said "Fixed" but the `## B<n>.` header lacked the `~~[OPEN]~~ -> FIXED` marker. Drift was visible in the open-index hand-written at the top of bugs.md - bugs that had been closed days ago (B8, B10, B11, B13, B14, B28) were still listed as open. All six were closed in this session's commits `f658963`, `a5d8ef4`, `75570f8` on `docs`.

**Root cause:** Two sources of truth per entry (header marker AND Status line) + no machine check. When they disagree, nothing catches it.

**Fix - R28 (NEW, next working session top priority after priorities 0-1):** Auto-generated open-bugs index + consistency validator.
- `tools/bugs_index.py` parses `bugs.md`, classifies each `## B<n>` / `## J<n>` header as OPEN / FIXED / WONT-FIX by header marker only (the declared canonical source), rewrites the block between `<!-- BEGIN_OPEN_INDEX -->` and `<!-- END_OPEN_INDEX -->` sentinels.
- Validator fails the commit if any Status line says "Fixed" / "Won't-fix" / "Resolved" without the matching header marker.
- Wire into the R27 pre-commit hook: if `bugs.md` is staged, run the script, re-stage if changed, block the commit if validation fails.
- Estimated ~100 lines of Python + one line in the pre-commit hook. One session of work.
- File as R28 in `roadmap.md` with full scope before starting. Ship as one commit.

## ORIGINAL "buggy mousetrap" directive (2026-04-23, still in force)
...
**Recently closed:**
- **Bug tracking drift sweep (2026-04-24 AM):** Closed six bugs that had "Fixed" Status lines but missing header markers: B8 (private-event filter, fixed 2026-04-18), B10 (venue duplicated, fixed 2026-04-18), B11 (unique breadcrumbs, fixed 2026-04-18, re-verified live), B13 (red date badge square, fixed 2026-04-18 two-pass to 80x80, re-verified live), B14 (Share CTA + reorder, fixed 2026-04-18), B28 (sync task scheduler installer, fixed 2026-04-22 via -marketing PR #3). Added hand-written "Currently open (quick index)" section at top of `bugs.md` as interim relief. **R28 filed** to automate this going forward - auto-generated index + pre-commit consistency validator; top priority for next working session. Commits [`f658963`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/f658963), [`a5d8ef4`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/a5d8ef4), [`75570f8`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/75570f8) on `docs`.
- **B1 + B7 parallel-run verification closed (2026-04-24 AM):** Day 3 of the 3-day window clean. Windows box fired 8:00 AM Central sync commit [`ce7fd4c`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/ce7fd4c) at 08:00:14 -0500. Old Perplexity cron in inaccessible thread continues to fire daily 13:11 UTC but is harmless because `sync_calendar.py` is gone from gh-pages; `sync_lib.py` is pure library with no side effects. Moved legacy Perplexity cron to "Deleted / defunct tasks" in `architecture/scheduled-tasks.md` as DEFUNCT-still-firing. Commit [`3d3918a`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/3d3918a) on `docs`.
- **R27 pre-commit hook shipped (2026-04-24 AM):** Option A only. Python pre-commit hook at `.githooks/pre-commit` (95 lines, ASCII-clean) blocks commits where `shows.json` is staged without any `shows/*.html`. Escape hatch: `SKIP_SHOWS_JSON_CHECK=1 git commit ...`. Top-level README updated with one-time install step (`git config core.hooksPath .githooks`). 6/6 test scenarios pass. Ray confirmed he ran the install command on the Windows box. Sandbox clones pick up the hook automatically via tracked `.githooks/` dir. Commit [`da829f3`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/da829f3) on `master`; docs update [`32b071a`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/32b071a) on `docs`. Option B (rebuild helper script) deferred - not needed while sync pipeline is the only writer to shows.json.
- **Critical platform context (2026-04-24 AM, NOT a project fix):** Perplexity Computer silently drops on submit in new threads; Ray cannot start a new session since ~2026-04-17. Reproduced across 2 browsers, 2 profiles, Windows client. Screenshots attached to engineering diagnostic ticket `b8614962-38d8-419c-90e0-fc8b610e5e16`. Support unresponsive 3+ weeks. Ray drafted final demand email + chargeback letter to send if silence continues past 2026-05-01. The project itself is unaffected - Windows cron keeps firing, site keeps serving, all work remains in GitHub.
