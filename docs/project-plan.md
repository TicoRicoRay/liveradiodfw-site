...
_Last updated: 2026-04-21 · 1:05 PM Central_
...
**Context (2026-04-21 session):** **B7 Part 2 shipped end-to-end.** `sync_calendar.py` split into `sync_lib.py` (pure functions on `gh-pages`, rename preserved git history) + `sync_runner.py` (orchestration + secrets, off-repo, destined for Ray's Windows Task Scheduler). `fetch_historic.py` deleted as a sixth caller with no future use. 5 remaining callers updated to `from sync_lib import …` and all 49 tests green. Exposure verified closed: `https://www.liveradiodfw.com/sync_calendar.py` → 404, GitHub raw → 404, passphrase no longer grep-able anywhere public. Ray's handoff package (`sync_runner.py`, `.env.example`, `windows-sync-task.md`) is in workspace as `b7-windows-handoff.zip`.

**Awaiting from Ray:** install on the Windows box (steps 1–7 of [runbooks/windows-sync-task.md](runbooks/windows-sync-task.md)), then 3-day parallel-run check between the new Windows task and the old Perplexity cron, then delete the Perplexity task from the "More Band Marketing" thread. After that B7 and B1 both close in the same motion (Windows Task Scheduler's local-time trigger is DST-safe by construction). Until the Perplexity cron is retired, the daily sync is double-firing — fine because both produce the same idempotent commit shape, the downside is Ray is paying Perplexity credits for the duplicate runs for 3 days.

**B7 Part 2 mid-flight deliverables (all shipped):**
- `gh-pages` commit [`b125405`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/b125405) — library split + 5 caller updates + 2 file deletions.
- `docs` commit [`aca5b21`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/aca5b21) — Windows install runbook.
- `docs` commit [`3fc9e33`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/3fc9e33) — R23 filing + scheduled-tasks.md row 3 for the Monthly Profile Audit.

**Second cron discovered and filed as R23 (2026-04-21):** Ray walked Jarvis through the Perplexity Tasks UI and surfaced TWO crons in the "More Band Marketing" thread: the known daily calendar sync (~19h to next fire) AND a previously-uninventoried **Monthly Profile Audit** (venue discovery → Mailchimp Venues audience `97cca06eff`, next fire ~2026-05-01). Mailchimp API confirmed it is working (15 `other_adds` on 2026-03-30 = monthly batch). The script itself lives only in that thread's task prompt — never extracted, never committed, loss risk is real. Filed as **R23** with plan to forensics-dump, extract to `liveradiodfw-marketing/venue_discovery/discover_venues.py`, then migrate to Ray's Windows Task Scheduler alongside `sync_runner.py` as the second customer of that infrastructure. Also added row 3 to `architecture/scheduled-tasks.md` so the cron is now on the durable inventory regardless of what happens to the owning thread. **This was the first band-marketing project Ray built with Jarvis — preserving it matters.**

**Hunt the Perplexity Tasks UI, not `schedule_cron(list)`:** Reaffirmed J1 the hard way this session. `schedule_cron(list)` returns empty from any thread that does not own the task. The way to find band crons is: Perplexity in-app → Tasks view (left nav, above History/Spaces/Customize/Computer as of 2026-04-21) → identify owning thread. Documented in `architecture/scheduled-tasks.md` under each cron's "How to find" block.

**2026-04-19 session context (preserved for continuity):** pure infrastructure cleanup. Opened a Bark.com bid for Ree's 6/7/26 birthday party (file in local workspace, not yet sent by Ray). Then ran a three-item sweep: **R2 closed** (UptimeRobot live on free plan, HEAD-only 5-min checks, alerts to info@, dashboard password-protected; HEAD-only means we cannot catch soft failures like 200-with-broken-body, which is a known paid-tier constraint we are explicitly accepting for this low-traffic marketing site). **R3 closed** (sitemap of 112 URLs submitted to GSC; `/lander` re-index requested; `/home` deduplicated via a Cloudflare Page Rule — zone is still on the legacy UI, so it is Page Rules not Redirect Rules for now — `*liveradiodfw.com/home*` → 301 → `https://www.liveradiodfw.com/` with preserve-query-string on; verified clean via curl across 7 variants covering apex/www, trailing slash, `.html`, query strings, case). **R9 closed** as a standing rule with two enforcement points: `audit_shows.py` catches CDT/CST in show data at build time, and a new **step 4 in `runbooks/end-of-session.md`** greps the session diff before every commit. First live test of the step-4 hook is this very commit. Step 4 surfaced real CDT strings at `gh-pages/sync_calendar.py:677` and `:731` in alert email bodies, which became **B17** (filed, not fixed — Ray will pick option from entry). New memory entry saved: SEO belt-and-suspenders preference (wildcard patterns covering apex/www, trailing slashes, file extensions, query strings) — applies to this project and all future web work.
...
1. **Install the Windows Task Scheduler sync on Ray's box.** Zip lives in workspace as `b7-windows-handoff.zip` (three files: `sync_runner.py`, `.env.example`, `windows-sync-task.md`). Walk steps 1–7 of [runbooks/windows-sync-task.md](runbooks/windows-sync-task.md). Expected 30 minutes of hands-on time. Then 3-day parallel-run verification, then delete the Perplexity "LiveRadioDFW Daily Calendar Sync" task from the "More Band Marketing" thread. B7 and B1 both close when the Perplexity task is retired.

2. **R23 - Preserve the Monthly Profile Audit venue-discovery cron.** See [roadmap.md#r23](roadmap.md). Filed 2026-04-21. Next fires ~2026-05-01; that is the last easy checkpoint before another month of drift. Forensics the "More Band Marketing" Perplexity thread for the task prompt BEFORE the thread is lost. First band-marketing project Ray built with Jarvis — do not lose. The Windows box from priority #1 is where it will migrate to once extracted.

3. **B1 - DST-safe sync cron.** See [bugs.md#b1](bugs.md). **Auto-closes when priority #1 lands** — Windows Task Scheduler uses local Central time with automatic DST handling. No separate work needed; just the cleanup of moving the old Perplexity row into 'Deleted / defunct tasks' in `architecture/scheduled-tasks.md`.
4. **R22 - Station / Theme / Show taxonomy + named-show build-out.** See [roadmap.md#r22](roadmap.md). High-priority per Ray (2026-04-19 `/lander` smell-test). Two linked problems: (a) the public-facing word for genre buckets vs. named themed products is unresolved — is "Station" what people search for and scan in nav? (b) A Tribute To MTV, Graduation Night, Texas Music Night, and other ready-to-book shows Ray has already forgotten live only in the marketing repo and memory — needs inventory + runbook + pages. Architecture discovery during filing: `gh-pages/stations.json` is orphaned (no file references it). Strongly recommend R11 ships first so nav changes are single-file.
5. **D1 - ChatGPT site audit.** See "Pending discussion" below. Ray has audit findings to walk through before they become bugs or roadmap items.
6. **R11 - head-level include extraction.** See [roadmap.md#r11](roadmap.md). High-leverage foundation item - makes future template-level fixes (like B12) single-file edits instead of 15-file sweeps. Ship before B12. Also a prerequisite for R22's taxonomy-driven nav changes.
...
8. **B6 - Videos two-click bug.** See [bugs.md#b6](bugs.md). Investigation-heavy; not a quick fix.
9. **R18 - Locale SEO landing pages.** See [roadmap.md#r18](roadmap.md). "Band-type in {city}" intent capture, leveraging R16 historic calendar as proof-of-presence. Medium-high leverage; queue behind foundation work and R4.
10. **R21 - Verify /home 301 consolidation.** See [roadmap.md#r21](roadmap.md). Fact-finding only, earliest check 2026-05-03. Natural pair with R4 reassessment on the same date.

**Recently closed:**
- **B7 Part 2 shipped — exposure closed end-to-end (2026-04-21):** `sync_calendar.py` removed from `gh-pages`. New shape: `sync_lib.py` (pure functions, repo) + `sync_runner.py` (orchestration + secrets, off-repo, Windows Task Scheduler). `fetch_historic.py` deleted as a sixth caller with no future use. 5 callers updated to `from sync_lib import …`; all 49 tests green. Verified: `https://www.liveradiodfw.com/sync_calendar.py` → 404, GitHub raw → 404, passphrase no longer grep-able anywhere public (30-second window from push to Cloudflare edge propagation). Handoff package (`sync_runner.py` + `.env.example` + `windows-sync-task.md`) is in workspace as `b7-windows-handoff.zip`. Commits [`b125405`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/b125405) on `gh-pages` and [`aca5b21`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/aca5b21) on `docs`. Install + 3-day parallel run + retire Perplexity cron is the only remaining work (tracked as priority #1 above).
