...
_Last updated: 2026-04-21 · PM Central (B7 runner published to `-marketing`; no-zip handoff pattern adopted)_
...
**Context (2026-04-21 session):** **B7 Part 2 shipped end-to-end.** `sync_calendar.py` split into `sync_lib.py` (pure functions on `gh-pages`, rename preserved git history) + `sync_runner.py` (orchestration + secrets, now published to the public [`liveradiodfw-marketing`](https://github.com/TicoRicoRay/liveradiodfw-marketing) repo at commit [`ee94c10`](https://github.com/TicoRicoRay/liveradiodfw-marketing/commit/ee94c10)). `fetch_historic.py` deleted as a sixth caller with no future use. 5 remaining callers updated to `from sync_lib import …` and all 49 tests green. Exposure verified closed: `https://www.liveradiodfw.com/sync_calendar.py` → 404, GitHub raw → 404, passphrase no longer grep-able anywhere public. **No zip handoff** — Ray's standing architectural preference (2026-04-21 direct quote: _"Rather than download zip files (like a caveman) - can we isolate any sensitive information in a single file - download it. Then everything else is just a pull from github as needed?"_). Only `.env` is hand-managed locally; everything else is `git clone` once, `git pull` forever.

**Awaiting from Ray:** `git clone https://github.com/TicoRicoRay/liveradiodfw-marketing.git` into `C:\Tools\LiveRadioDFW\liveradiodfw-marketing\`, copy `.env.example` → `.env` and fill in the five keys from 1Password, run `.\setup_sync_task_scheduler.ps1` as Administrator. Full runbook ships with the code at [`liveradiodfw-marketing/docs/windows-sync-task.md`](https://github.com/TicoRicoRay/liveradiodfw-marketing/blob/master/docs/windows-sync-task.md). Then 3-day parallel-run check between the new Windows task and the old Perplexity cron, then delete the Perplexity task from the "More Band Marketing" thread. After that B7 and B1 both close in the same motion (Windows Task Scheduler's local-time trigger is DST-safe by construction). Until the Perplexity cron is retired, the daily sync is double-firing — fine because both produce the same idempotent commit shape, the downside is Ray is paying Perplexity credits for the duplicate runs for 3 days.

**B7 Part 2 deliverables (all shipped):**
- `-site/gh-pages` commit [`b125405`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/b125405) — library split + 5 caller updates + 2 file deletions.
- `-site/docs` commit [`aca5b21`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/aca5b21) — earlier runbook draft (since retired — runbook now lives in `-marketing` as canonical source).
- `-site/docs` commit [`3fc9e33`](https://github.com/TicoRicoRay/liveradiodfw-site/commit/3fc9e33) — R23 filing + scheduled-tasks.md row 3 for the Monthly Profile Audit.
- `-marketing/master` commit [`ee94c10`](https://github.com/TicoRicoRay/liveradiodfw-marketing/commit/ee94c10) — `sync_runner.py`, `.env.example`, `.gitignore`, `docs/windows-sync-task.md`, `setup_sync_task_scheduler.ps1`.
...
1. **Install the Windows Task Scheduler sync on Ray's box.** `git clone https://github.com/TicoRicoRay/liveradiodfw-marketing.git` into `C:\Tools\LiveRadioDFW\liveradiodfw-marketing\`, copy `.env.example` → `.env` and fill in the five keys from 1Password, run `.\setup_sync_task_scheduler.ps1` as Administrator. Full runbook: [`liveradiodfw-marketing/docs/windows-sync-task.md`](https://github.com/TicoRicoRay/liveradiodfw-marketing/blob/master/docs/windows-sync-task.md). Expected ~15 minutes hands-on. Then 3-day parallel-run verification, then delete the Perplexity "LiveRadioDFW Daily Calendar Sync" task from the "More Band Marketing" thread. B7 and B1 both close when the Perplexity task is retired.
