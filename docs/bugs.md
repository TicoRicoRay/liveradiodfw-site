...
**Follow-up 2026-04-21 PM (later, post-B25 merge):** Checked the band inbox `info@liveradiodfw.com` via Outlook connector (per band-email routing rule: all LRDFW notifications go to `info@`, not `rmyers@futurebright.com`). Queries `from:perplexity.ai`, `from:support@perplexity.ai`, `Perplexity support`, `Perplexity ticket`, `Perplexity credits`, `Perplexity scheduled task` all returned zero results. No ticket response has arrived at `info@liveradiodfw.com`. Two plausible explanations: (a) tickets were filed from Ray's personal / work account (e.g. `rmyers@futurebright.com` or another address, in which case the reply would land there, not in the band inbox), or (b) Perplexity support replies live in-product (support-ticket UI or notifications panel) rather than email. Next follow-up actions, in order of leverage:
...
**Stopgap (shipped 2026-04-21 PM, commit [`afcfe45`](https://github.com/TicoRicoRay/liveradiodfw-marketing/commit/afcfe45) on `-marketing`):** Replaced the `subprocess_exec` call with a stub that prints the intended email contents to stdout (Subject / To / Body) and returns `True` so callers treat the alert as delivered. The change summary is still visible in `sync.log` (Task Scheduler wrapper captures stdout), and the actual commit shape is always visible in `-site` `gh-pages` git log. **The stopgap is NOT cosmetic — it silences the watchdog (see Purpose below). Treat this as a bug that masks a safety system, not a missing nicety.**

**Purpose of the alert email (per Ray, 2026-04-21 PM):** The email is **two jobs in one envelope**:
1. **Friendly heads-up** — "hey, the cron ran, here's what changed." Ray reads it over coffee and has situational awareness of what the site looks like today without touching git.
2. **Human-in-the-loop watchdog for bad cron behavior** — if the sync does something wrong (deletes 9 shows because the calendar API returned an empty list, regenerates every page with mojibake, pushes a private-event address to the public site, flips every show to "Free", etc.), the email is the single trigger that gets a human's eyes on it before damage compounds. Without it, a bad push sits live on `liveradiodfw.com` until Ray happens to look. Cron jobs that push to production need a human tripwire; this email IS the tripwire.

Implication for the fix: the email can't just send on success — it must **always send when `sync_runner.py` completes a run that changed anything**, regardless of whether the change was expected or suspicious. The body needs enough signal for Ray to eyeball "yes this is fine" vs. "wait, why did 9 shows disappear" in 10 seconds. Suggested body shape:
- Run timestamp + host + commit SHA pushed (or "no changes" if clean)
- Count of events fetched, gigs kept, private filtered
- **Diff summary**: file-by-file added/modified/deleted line counts from the git push (`git diff --stat HEAD~1 HEAD`)
- Any warnings the parser raised (missing ticket price, missing description, unparseable address, etc.)
- A "🚩 REVIEW" prefix in the subject if >N shows changed in a single run (pick a threshold; 3+ changed shows in one sync is suspicious on a 10-gig calendar), else a neutral prefix.

**Proper fix (this bug):** Restore real mail delivery from the Windows box **with a body rich enough to serve as a watchdog, not just a "ran OK" receipt**. Options:
...
**Why this matters:** The cron job **pushes directly to production** (`gh-pages` is what `liveradiodfw.com` serves). Any corruption the sync introduces — bad calendar parse, mojibake regression, accidental deletion, private-event leak — is live on the public site the moment the push completes. The alert email is the **only human tripwire** between a bad cron run and Ray noticing organically (which could be days). It also catches the softer failures: missing descriptions, missing ticket prices, newly-imported shows that need hand-curation. B16/B16.2 (description drafts pipeline) depends on this loop; B18 surfaced specifically because the email warned us. **Reclassifying urgency accordingly:** not a nice-to-have, this is production safety.
